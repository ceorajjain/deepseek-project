"""Stage 15: Gujarat Metro (GMRC — Ahmedabad + Surat) awarded construction + winners.

gujaratmetrorail.com/awarded-tenders lists winner + work-order date directly.
Granite target = station/viaduct+station/depot/building/MMI construction.
"""
import json
import os
import re
...
log = get_logger("15_gujarat_metro")

URL = "https://www.gujaratmetrorail.com/awarded-tenders/"
CACHE = os.path.join(config.STATE, "gujarat_metro_cache", "awarded_tenders.html")
ctx = ssl.create_default_context()
ctx.check_hostname = False
ctx.verify_mode = ssl.CERT_NONE

GRANITE_NEG = re.compile(
    r"o&m|maintenance|supply|rate contract|camc|refill|spares|plantation|geotech|"
    r"topograph|salvage|disposal|printing|stationery|transportation|loading|unloading|"
    r"mock-up|security|customer facilitation|noise|vibration|consultanc|survey|"
    r"hvac|escalator|lift|signage|cleaning|housekeeping|software|trend|warranty|printer|"
    r"train wrapping|road resurface|utility shifting|s&t|ibs|signalling|telecom|advertis",
    re.I,
)
GRANITE_POS = re.compile(
    r"construction|station|viaduct|depot|building|civil|architectural|finishing|"
    r"entry|exit|stabling yard|multi modal|underground|housing|pier|foundation|tunnel|"
    r"sundry items|corridor|workshop",
    re.I,
)
...
def add_winner(cur, project_id, company_id, date, source_url):
    ...
    """, (project_id, company_id, source_url,
          json.dumps({"source_type": "official_award_fact", "source_url": source_url,
                      "work_order_date": date, "actor": config.ACTOR})))
    return True
...
def main():
    ...
    for tb in soup.find_all("table"):
        for tr in tb.find_all("tr"):
            ...
            if len(cells) < 5:
                continue
            tn, work, proj, winner, date = cells[:5]
            ...
            if not wname:
                continue
            title = (work + " | " + proj)[:500]
            pid, created = find_or_create_project(cur, "gujarat_metro", tn, title,
                                                  tn, URL, None, None)
            cur.execute("""
            ...
            if add_winner(cur, pid, cid, date, URL):
                winners += 1
            log.info("%s: winner=%s", tn[:38], wname[:42])
    conn.commit()
    conn.close()
    log.info("Gujarat Metro done: granite_projects=%d winners_added=%d", projects, winners)
